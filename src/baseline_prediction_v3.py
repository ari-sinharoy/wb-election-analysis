...
sir_rates = pd.read_csv(os.path.join(dpath, "sir_constituency_rates.csv"),
                        header = 0)
...
def sir_simple_adjust(df, sir_weights=None):
    """
    Simple SIR-only adjustment.

    Logic:
    deleted_party_votes =
        total_electors * sir_reduction_rate * party_vote_share * party_weight

    adjusted_party_votes =
        original_party_votes - deleted_party_votes

    Then convert back to vote share.
    """
    ...
    # Safety
    df_adj["sir_reduction_rate"] = pd.to_numeric(
        df_adj["sir_reduction_rate"],
        errors="coerce"
    ).fillna(0.0)

    # Convert 20 -> 0.20 if needed
    df_adj.loc[df_adj["sir_reduction_rate"] > 1, "sir_reduction_rate"] /= 100

    df_adj["sir_reduction_rate"] = df_adj["sir_reduction_rate"].clip(0, 1)

    df_adj["total_electors_estimated"] = pd.to_numeric(
        df_adj["total_electors_estimated"],
        errors="coerce"
    )

    # Total deleted electors
    df_adj["sir_deleted_electors"] = (
        df_adj["total_electors_estimated"] * df_adj["sir_reduction_rate"]
    )

    for p in parties:
        share_col = vote_share_cols[p]

        # Original party votes
        df_adj[f"{p}_base_votes"] = (
            df_adj["total_electors_estimated"] * df_adj[share_col] / 100
        )

        # SIR-deleted votes assigned to this party
        df_adj[f"{p}_sir_deleted_votes"] = (
            df_adj["sir_deleted_electors"]
            * df_adj[share_col] / 100
            * sir_weights[p]
        )

        # Adjusted party votes
        df_adj[f"{p}_sir_adj_votes"] = (
            df_adj[f"{p}_base_votes"] - df_adj[f"{p}_sir_deleted_votes"]
        ).clip(lower=0)

    adj_vote_cols = [f"{p}_sir_adj_votes" for p in parties]

    df_adj["sir_adjusted_total_votes"] = df_adj[adj_vote_cols].sum(axis=1)

    for p in parties:
        df_adj[f"{p}_sir_adj_share"] = (
            100 * df_adj[f"{p}_sir_adj_votes"] / df_adj["sir_adjusted_total_votes"]
        )

    # Optional: adjusted electors after SIR
    df_adj["sir_adjusted_electors"] = (
        df_adj["total_electors_estimated"] - df_adj["sir_deleted_electors"]
    ).round().astype("Int64")
...
df = sir_simple_adjust(df)

df = df.merge(
    mapping_df,
    on=["constituency","ls_constituency"],
    how="left"
)


LS_result_2014 = {'tmc': 0.3935, 'bjp': 0.1684,
                      'left': 0.2961, 'congress': 0.0958,
                      'others': 0.026}
...
# we will add 50% weight to the lok sabha swings 
lsmult = 0.5

df["pred_tmc_vote_share"] = df["tmc_sir_adj_share"] + df["tmc_swing"] * lsmult
df["pred_bjp_vote_share"] = df["bjp_sir_adj_share"] + df["bjp_swing"] * lsmult
df["pred_left_vote_share"] = df["left_sir_adj_share"] + df["left_swing"] * lsmult
df["pred_congress_vote_share"] = df["congress_sir_adj_share"] + df["congress_swing"] * lsmult
df["pred_others_vote_share"] = df["others_sir_adj_share"]
...
df["pred_total"] = df[pred_cols].sum(axis=1)

for col in pred_cols:
    df[col] = 100 * df[col] / df["pred_total"]
...
sorted_adj_votes = np.sort(df[adj_vote_cols].values, axis=1)

df["sir_adjusted_margin_pct"] = (
    sorted_adj_votes[:, -1] - sorted_adj_votes[:, -2]
)


#df.to_csv("data/processed/sir_adjusted_predictions.csv", index=False)
...
seat_counts = df["sir_adjusted_winner"].value_counts()
...
close_seats = df[df["sir_adjusted_margin_pct"] < 3].copy()
